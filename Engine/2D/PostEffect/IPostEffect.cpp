...
ID3D12Device* IPostEffect::device_;

ID3D12GraphicsCommandList* IPostEffect::commandList;

IPostEffect::VertexPosUv IPostEffect::vertices[4];

IPostEffect::VertexPosUv* IPostEffect::vertMap;

Microsoft::WRL::ComPtr<ID3D12Resource> IPostEffect::vertBuff;	//頂点バッファ

//頂点バッファビューの作成
D3D12_VERTEX_BUFFER_VIEW IPostEffect::vbView{};
Microsoft::WRL::ComPtr<ID3D12Resource> IPostEffect::texBuff[2];

Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> IPostEffect::descHeapSRV;
//深度バッファ
Microsoft::WRL::ComPtr<ID3D12Resource> IPostEffect::depthBuff;
//RTV用のデスクリプタヒープ
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> IPostEffect::descHeapRTV;
//DSV用のデスクリプタヒープ
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> IPostEffect::descHeapDSV;

Microsoft::WRL::ComPtr<ID3D12PipelineState> IPostEffect::pipelineState;
Microsoft::WRL::ComPtr<ID3D12RootSignature> IPostEffect::rootSignature;

// 定数バッファ
Microsoft::WRL::ComPtr<ID3D12Resource> IPostEffect::constDataBuff_;
// マッピング済みアドレス
IPostEffect::SendDataGPU* IPostEffect::dataMap = nullptr;

const float IPostEffect::clearColor[4] = { 0.0f,0.0f,0.0f,0 };

const std::wstring IPostEffect::kDefaultShaderDirectoryPath = L"Resources/SHADER/POSTFX/";
const std::wstring IPostEffect::kShaderExtention = L".hlsl";

IPostEffect::IPostEffect()
{
}

void IPostEffect::Initialize(DirectXCommon* dxCommon, const std::wstring& fileName)
{
	HRESULT result;
	...
	CreatGraphicsPipeline(fileName);
	...
	// シェーダー番号
	dataMap->shadeNumber = 2;

	constDataBuff_->Unmap(0, nullptr);
}

void IPostEffect::Finalize()
{
...
void IPostEffect::CreatGraphicsPipeline(const std::wstring& fileName)
{
	HRESULT result;
	...
	const std::wstring FULLPATH_PS = kDefaultShaderDirectoryPath + fileName + L"PS" + kShaderExtention;
...
void IPostEffect::PreDrawScene(ID3D12GraphicsCommandList* cmdList, uint32_t TexNum)
{
	commandList = cmdList;

	CD3DX12_RESOURCE_BARRIER resouceBar = CD3DX12_RESOURCE_BARRIER::Transition(texBuff[TexNum].Get(),
		D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
	...
	rtvHs = CD3DX12_CPU_DESCRIPTOR_HANDLE(descHeapRTV->GetCPUDescriptorHandleForHeapStart(), (INT)TexNum,
...
void IPostEffect::Draw(ID3D12GraphicsCommandList* cmdList)
{
	commandList = cmdList;
...
void IPostEffect::PostDrawScene(uint32_t TexNum)
{
	CD3DX12_RESOURCE_BARRIER RESOURCE_BARRIER = CD3DX12_RESOURCE_BARRIER::Transition(texBuff[TexNum].Get(),
